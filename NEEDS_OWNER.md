...
## ★ 1) الكابتشا مطفية الآن = لا حماية بوتات — أعِدها قبل الإعلانات
أوقفتَ كابتشا Supabase الليلة لفكّ حاجب «ما قدرنا نبدأ» (كان صحيحًا للفكّ). لكن قبل تشغيل إعلانات، فعّلها صح وإلا البوتات تستنزف Serper + سقف المجهولين:
- Cloudflare Turnstile: سجّل النطاق `app.dabbir.app` على مفتاح الموقع `0x4AAAAAAEb1TMMpcUpbSQUO`، واحصل على الـSecret Key.
- Supabase → Auth → Captcha: المزوّد Turnstile + الصق الـSecret Key.
- الوركر: `npx wrangler secret put TURNSTILE_SECRET_KEY --config wrangler.production.jsonc` (الصق نفس السرّ).
...
## ★ 4) سرعة البحث ~80 ثانية — تجزّؤ الوقت + خطة
تجميد الواجهة عند «دوّر لي» أُصلح سابقًا (flushSync، <100ms). الـ80 ثانية هي زمن **الخادم**: نداءات Serper المتعددة (عدة مصادر × عدة استعلامات) + قراءة gpt-4o لكل مزوّد. الخطة الآمنة (جلسة قصيرة): توازي نداءات Serper بدل التسلسل، حدّ المصادر/الاستعلامات لما يكفي، الدليل الخاص أولًا للطلبات المكررة، كاش أذكى. لم أغيّرها الليلة لأن تعديل مسار البحث بلا نشر-تحقّق يخاطر بكسره.
